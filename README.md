...
Para poder utilizarlo lo hemos tenido que instalar en la máquina virtual que ejerce de servidor. Para ello, nos hemos conectado via SSH a dicha máquina y hemos escrito el comando: "sudo apt update" para poder mantener el sitema y actualizar los servicios. Para que nos entendamos mejor, es como actualizar el catálogo de una tienda para saber qué productos tiene disponibles y sus versiones más recientes, sin embargo, este no instala ni actualiza ningún programa, solo se actualiza la información. Posteriormente, vamos a utilizar el comando "sudo apt install -y ca-certificates curl gnupg" para instalar las nuevas dependencias del sistema y los comandos "sudo install -m 0755 -d /etc/apt/keyrings", "curl -fsSL https://download.docker.com/linux/debian/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg" y "sudo chmod a+r /etc/apt/keyrings/docker.gpg" para instalar  la clave GPG oficial de docker.<br>
<br>Ahora, procederemos a agregar el repositorio oficial de docker con el siguiente comando: "echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/debian trixie stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null" que, para quién no entienda qué es un repositorio es donde se almacenan, organizan, preservan y gestionan archivos, documentos o datos concretos del proyecto, en este caso, los de docker. Finalmente, instalaremos docker con el siguiente comando: "sudo apt install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin".<br>
<br>Para crear los contenedores del docker en sus carpetas correspondientes, tal y como podemos observar en la siguiente imágen:

<br><br> enlace imagen<br>

<br> Deberemos ejecutar los siguientes comandos para crear sus respectivos contenedores empezando por el denominado dns-dhcp:
docker run -d --name dns-dhcp --restart unless-stopped -v $(pwd)/dns-dhcp:/etc/pihole -v $(pwd)/dns-dhcp/dnsmasq:/etc/dnsmasq.d -e TZ=Europe/Madrid -e WEBPASSWORD=pentaLink \
  -e PIHOLE_DNS_=8.8.8.8;8.8.4.4 --network host pihole/pihole:latest<br>
<br>Para el contenedor denominado web:
docker run -d --name web --restart unless-stopped -v $(pwd)/web:/var/www/html -p 80:80 -e TZ=Europe/Madrid php:8.2-apache
<br>Para la base de datos:
docker run -d --name database --restart unless-stopped -v $(pwd)/database:/var/lib/mysql -e TZ=Europe/Madrid -e MYSQL_ROOT_PASSWORD=pentaLink \
  -e MYSQL_DATABASE=pentaLink -e MYSQL_USER=pentaLink -e MYSQL_PASSWORD=pentaLink -p 3306:3306 mariadb:latest
 <br>Por último, el contenedor de configuración:
 docker run -d --name config --restart unless-stopped -v $(pwd)/config:/app/config -e TZ=Europe/Madrid alpine:latest sleep infinity<br>
<br>Resumidamente, para aquellos que no entiendan qué quiere decir el comando lo vamos a explicar detalladamente, empezando por "docker run -d" sirve para crear y ejecutar el contenedor que se está creando, "--name "nombreContendor" " sirve para darle nombre al contenedor que vamos a crear, "--restart unless-stopped" quiere decir que queremos que el contenedor se reinicie a menos que esté parado, "-v $(pwd)..." sirve para conectar una carpeta de la máquina virtual con una carpeta dentro del contenedor, "-p xx:xx" sirve para indicar el puerto que utilizará, "-e TZ=Eurpoe/Masdrid sirve para indicar la zona horaria donde las demás variantes y cosas escritas en los comandos son detalles como las contraseñas y servicios (donde los explicaremos a continuación) que van a estar utilizando cada uno de los contenedores.
...
#### Incidencia 2 - Transferencia del proyecto a otro PC
